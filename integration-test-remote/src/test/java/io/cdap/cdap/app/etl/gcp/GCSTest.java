...
public class GCSTest extends DataprocETLTestBase {
  ...
  public void testSchemaDetectionOnSingleFile() throws Exception {
    ...
    String dataOne = "John;Doe;21";
    String dataTwo = "Alice;Green;20";
    ...
    ApplicationManager appManager = deployApplication(appId, appRequest);
    startWorkFlow(appManager, ProgramRunStatus.COMPLETED);
    List<String> blobNames = listBucket(sinkBucket, "output");

    // Value 20 is of integer data type. Hence no quotes around the number.
    Assert.assertTrue(blobNames.size() > 0);
    assertBlobContains(sinkBucket, blobNames.get(0), "20");
    assertBlobNotContains(sinkBucket, blobNames.get(0), "\"20\"");
  }
