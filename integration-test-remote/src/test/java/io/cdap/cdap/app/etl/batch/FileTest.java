...
public class FileTest extends ETLTestBase {
    ...
    private String setupData(DirectoryType directoryType) throws Exception {
        ...
        switch(directoryType) {
            case MULTIPLE_FILES_WITH_HEADERS_AND_SIMILAR_SCHEMAS:
                Arrays.asList("test-data-1.csv", "test-data-2.csv")
                        .forEach(fileName -> uploadFile(
                                fileName,
                                "multiple-files-with-headers-and-similar-schemas/",
                                serviceURL));
                break;

            case MULTIPLE_FILES_WITH_HEADERS_AND_DIFFERENT_SCHEMAS:
                Arrays.asList("test-data-3.csv", "test-data-4.csv")
                        .forEach(fileName -> uploadFile(
                                fileName,
                                "multiple-files-with-headers-and-different-schemas/",
                                serviceURL));
                break;

            case MULTIPLE_FILES_WITHOUT_HEADERS_AND_SIMILAR_SCHEMAS:
                Arrays.asList("test-data-5.csv", "test-data-6.csv")
                        .forEach(fileName -> uploadFile(
                                fileName,
                                "multiple-files-without-headers-and-similar-schemas/",
                                serviceURL));
                break;
        }
        return sourcePath;
    ...
    public void testFirstAutomaticSchemaDetection() throws Exception {
        ...
        */
        ...
        try {
            ...
            assertContains(e.toString(), "java.lang.IllegalArgumentException");
        }
    ...
    public void testSecondAutomaticSchemaDetection() throws Exception {
        ...
        // Here we prove "test-data-6.csv" is also read
        Row sixthRow = table.get(Bytes.toBytes(6));
        Assert.assertEquals("washington dc", sixthRow.getString("body_1"));
    }

    @Test
    public void testThirdAutomaticSchemaDetection() throws Exception {
        /*
    ...
    private boolean assertContains(String l, String r) {
        return l.contains(r);
    }
}
